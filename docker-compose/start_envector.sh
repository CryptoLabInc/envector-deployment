...
export START_ENVECTOR_COMPOSE_DIR="${COMPOSE_DIR}"
export START_ENVECTOR_REPO_ROOT="${REPO_ROOT}"
...
propagate_version_tag() {
  local version="$1"
  [[ -n "$version" ]] || return 0
  ensure_override "VERSION_TAG" "$version"
  ensure_override "ENVECTOR_ENDPOINT_TAG" "$version"
  ensure_override "ENVECTOR_BACKEND_TAG" "$version"
  ensure_override "ENVECTOR_ORCHESTRATOR_TAG" "$version"
  ensure_override "ENVECTOR_SHAPER_TAG" "$version"
  ensure_override "ENVECTOR_COMPUTE_TAG" "$version"
}
...
if [[ -n "$chosen_version" ]]; then
  effective_version_override="$chosen_version"
elif [[ -n "${VERSION_TAG:-}" ]]; then
  effective_version_override="${VERSION_TAG}"
fi
if [[ -n "$effective_version_override" ]]; then
  propagate_version_tag "$effective_version_override"
fi
